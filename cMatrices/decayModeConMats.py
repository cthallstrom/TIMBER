...
if readFile:
    ...
    for ievent in range(t.GetEntries()):
        ...
        for ijet in range(nJets):
            ...
            PNWMid = t.man_decays_TT

            # Fill truth info into all x-axis values
            if t.true_decays_TT[ijet] == 1:   #deciding on the truth, we can just look at the truth from our truth vector
                i = 0.5      
            elif t.true_decays_TT[ijet] == 2:
                i = 1.5
            elif t.true_decays_TT[ijet] == 3:
                i = 2.5
            elif t.true_decays_TT[ijet] == 4:
                i = 3.5
            elif t.true_decays_TT[ijet] == 5:
                i = 4.5
            elif t.true_decays_TT[ijet] == 6:
                i = 5.5
    ...
    histFile = TFile.Open(f"/uscms_data/d3/cai/run3VLQ/TIMBER/cMatrices/{name}_D.root", "recreate")
...
histFile = TFile.Open(f"/uscms_data/d3/cai/run3VLQ/TIMBER/cMatrices/{name}_D.root")
...
gPad.Update()
canv1.SaveAs(f"/uscms_data/d3/cai/run3VLQ/TIMBER/cMatrices/{name}_Decays.png")
