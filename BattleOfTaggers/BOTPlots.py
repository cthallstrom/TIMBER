...
def runAll(eff_types,ptbins,nbins,files):
    for eff in eff_types:
        createRootFiles(eff,files,ptbins,nbins)
#        plotEff(eff)

def createRootFiles(eff_type,file_strs,ptbins,nbins):
    for file_str in file_strs:
        ...
        for i,matched in enumerate({"tagged","mistagged"}):
            ...
            for j,part in enumerate({"t_quark","W_boson","Z_boson","H_boson"}):
                ...
                if eff_type == "mass":
                    histFile = TFile.Open(f"massEff/PNWM_{mass_name}_massEff.root", mode)
                    PNWM.Write()
                    histFile.Write()
                    histFile.Close()
                
                    histFile = TFile.Open(f"massEff/GPT_{mass_name}_massEff.root", mode)
                    GPT.Write()
                    histFile.Write()
                    histFile.Close()
                                
                    histFile = TFile.Open(f"massEff/GPTWM_{mass_name}_massEff.root", mode)
                    GPTWM.Write()
                    histFile.Write()
                    histFile.Close()
                elif eff_type == "algo":
                    histFile = TFile.Open(f"taggerEff/{mass_name}_taggerEff.root", mode)
                    PNWM.Write()
                    GPT.Write()
                    GPTWM.Write()      
                    histFile.ls()
        ...
        for part in {"t_quark","W_boson","Z_boson","H_boson"}:
            if eff_type == "mass":
                for tagger in {"PNWM","GPT","GPTWM"}:
                    ...
                    tagger_file = TFile.Open(f"massEff/{part}_{matched}_{tagger}_massEff.root","read")
                    tagger_file.ls()
                    mass_trees = [key.GetName() for key in tagger_file.GetListOfKeys() if key.GetClassName() == "TH1D"]
                ...
                for file_name in os.scandir("taggerEff"):
                    ...
                    if file_name[-5:] == ".root":
                        ...
                        print(f"{file_name} becomes {mass_name}")
...
runAll(eff_types,ptbins,nbins,files)
